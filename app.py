...
for test in test_items:
    for file in files:
        if test+'_' in file.lower():
            dict_df[test] = creat_df_3dmark(os.path.join(first_subfolder, file))
...
for dir in dirs[1:]:
    ...
    for test in test_items:
        for file in files:
            if test+'_' in file.lower():
                dict_df[test] = append_df_3dmark(os.path.join(subfolder, file), dict_df[test])
...
for test in test_items:
    ...
    dict_df[test].sort_values(by='Create Time', inplace=True, ascending=False, ignore_index=True)
    # 5. Shorten the cloumns by removing the test name
    cleanup_dict = {}
    for name in dict_df[test].columns:
        cleanup_dict[name] = re.sub(test.replace('-', ''), '', name, flags=re.IGNORECASE)
